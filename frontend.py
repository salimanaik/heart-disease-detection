...
def main():
    ...
    model_choice = st.sidebar.selectbox("Select Model to Visualize", 
                                       ['All Models', 'Decision Tree', 'Random Forest', 'Logistic Regression', 'SVM'])
    ...
    if 'results' in st.session_state:
        ...
        if predict_btn:
            ...
            for i, model_name in enumerate(['Decision Tree', 'Random Forest', 'Logistic Regression', 'SVM']):
                with eval(f"col{i+1}"):
                    prob = st.session_state.probabilities[model_name][0]
                    prediction = st.session_state.predictions[model_name][0]
                    risk_color = "🟢 Low" if prediction == 0 else "🔴 High"
                    st.metric(f"{model_name}", f"{prob:.1%}", f"{risk_color} Risk")
